...
a_imp_df <- readRDS("processed_data/analysis_data.rds") %>% 
  select(site, id, pex_7d) %>% 
  unnest(pex_7d)

a_imp_bl <- readRDS("processed_data/obj5_bl_7d.rds")
...
b_flag <- b_nest %>% 
  mutate(
    flag_tmpts = map(add_pex, ~ flag_exacerbation_timepoints(.x, type = "7d")),
...
# Save
write_csv(d_sum, "outputs/objs/05_responsiveness/summaries/proxies/sum_lf_all_7d.csv")
write_csv(d_sum_lt5, "outputs/objs/05_responsiveness/summaries/proxies/sum_recovery_lt5_7d.csv")
write_csv(d_sum_lt100, "outputs/objs/05_responsiveness/summaries/proxies/sum_recovery_lt100_7d.csv")
...
write_csv(
  g_sum_lt5_eventual,
  "outputs/objs/05_responsiveness/summaries/proxies/sum_eventual_recovery_lt5_7d.csv"
)

write_csv(
  g_sum_lt100_eventual,
  "outputs/objs/05_responsiveness/summaries/proxies/sum_eventual_recovery_lt100_7d.csv"
)
...
write_csv(
  h_reason_sum_pct,
  "outputs/objs/05_responsiveness/summaries/proxies/sum_nonrecovery_reasons_lt5_7d.csv"
)

write_csv(
  h_reason_sum_ml,
  "outputs/objs/05_responsiveness/summaries/proxies/sum_nonrecovery_reasons_lt100_7d.csv"
)
